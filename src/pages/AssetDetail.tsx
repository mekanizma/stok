...
import { ArrowLeft, Boxes, Tag, Calendar, DollarSign, Shield, FileText, ArrowRightLeft, Edit3, Trash2, MapPin, User as UserIcon, Building2, Package, Printer } from 'lucide-react';
...
export default function AssetDetail({ assetId, navigate, onRefresh, users }: Props) {
  ...
  const handleCheckin = async () => {
    if (!asset || !asset.assigned_to_id) return;
    await supabase.from('checkout_history').insert({
      asset_id: asset.id,
  ...
  const generatePDF = () => {
    ...
    const givenByName = lastCheckout?.assigned_to ? `${lastCheckout.assigned_to.first_name} ${lastCheckout.assigned_to.last_name || ''}` : '—';
    ...
    const rows: [string, string][] = [
      ...
      [t('category'), asset.category?.name || '—'],
      [t('defaultLocation'), locName],
      [t('purchaseDate'), asset.purchase_date ? formatDate(asset.purchase_date) : '—'],
      [t('purchaseCost'), asset.purchase_cost ? `${asset.purchase_cost.toLocaleString()}` : '—'],
      [t('warranty'), asset.warranty_months ? `${asset.warranty_months} ${lang === 'tr' ? 'ay' : 'months'}` : '—'],
    ];

    const assigneeRows: [string, string][] = assignee ? [
      [t('fullName'), `${assignee.first_name} ${assignee.last_name || ''}`],
      [t('title'), assignee.job_title || '—'],
      [t('email'), assignee.email || '—'],
      [t('phone'), assignee.phone || '—'],
      [t('employeeNumber'), assignee.employee_num || '—'],
      [t('department'), assignee.location?.name || '—'],
    ] : [];

    const html = `<!DOCTYPE html><html lang="${lang}"><head><meta charset="utf-8"><title>${t('deploymentDocument')}</title>
<style>
...
.header h1 { font-size:22px; color:#1e293b; text-transform:uppercase; letter-spacing:1px; }
...
<div class="header">
<div><h1>${t('deploymentDocument')}</h1></div>
...
${assignee ? `<div class="section"><h2>${t('assignedToInfo')}</h2><table class="info-table">${assigneeRows.map(([l,v]) => `<tr><td>${l}</td><td>${v}</td></tr>`).join('')}</table></div>` : ''}
...
<div class="sig-box"><div class="sig-label">${t('receivedBy')}</div><div class="sig-line"><div class="name">${assignee ? `${assignee.first_name} ${assignee.last_name || ''}` : '—'}</div>${t('nameSurname')} / ${t('signature')}</div></div>
</div>
<div class="footer">${t('assetTracker')} · ${t('deploymentDocument')} · ${docNo}</div>
<button class="print-btn" onclick="window.print()">${t('printDocument')}</button>
  ...
  if (!asset) {
    return (
      <div className="p-6 text-center">
        ...
        <Button className="mt-4" onClick={() => navigate({ name: 'assets' })}>{t('backToAssets')}</Button>
      </div>
    );
  }
  ...
  const infoItems = [
    ...
    { icon: Tag, label: t('category'), value: asset.category?.name || '—' },
    { icon: MapPin, label: t('defaultLocation'), value: asset.default_location?.name || '—' },
    { icon: Calendar, label: t('purchaseDate'), value: asset.purchase_date ? formatDate(asset.purchase_date) : '—' },
    { icon: DollarSign, label: t('purchaseCost'), value: asset.purchase_cost ? `$${asset.purchase_cost.toLocaleString()}` : '—' },
    { icon: Shield, label: t('warranty'), value: asset.warranty_months ? `${asset.warranty_months} ${lang === 'tr' ? 'ay' : 'months'}` : '—' },
  ];

  return (
    <div className="p-4 lg:p-6 max-w-5xl mx-auto">
      <button onClick={() => navigate({ name: 'assets' })} className="flex items-center gap-2 text-sm text-gray-500 hover:text-gray-700 mb-4 transition-colors">
        <ArrowLeft className="w-4 h-4" /> {t('backToAssets')}
      </button>

      {/* Header card */}
      <div className="bg-white rounded-xl border border-gray-200 overflow-hidden mb-6">
        <div className="flex flex-col sm:flex-row">
          <div className="flex items-center justify-center w-full sm:w-48 h-48 bg-gradient-to-br from-slate-50 to-slate-100 shrink-0">
            {asset.image_url ? (
              <img src={asset.image_url} alt={asset.name} className="w-full h-full object-cover" />
            ) : (
              <div className="flex items-center justify-center w-20 h-20 rounded-2xl bg-white shadow-sm text-slate-400">
          ...
          <div className="flex-1 p-5">
            ...
            {asset.assigned_to ? (
              <div className="flex items-center gap-3 p-3 bg-blue-50 rounded-lg mb-4">
                <Avatar name={`${asset.assigned_to.first_name} ${asset.assigned_to.last_name || ''}`} />
                <div>
                  <p className="text-sm font-medium text-gray-900">{asset.assigned_to.first_name} {asset.assigned_to.last_name}</p>
                  <p className="text-xs text-gray-500">{asset.assigned_to.job_title || t('employee')} {asset.assigned_to.email ? `· ${asset.assigned_to.email}` : ''}</p>
                </div>
              </div>
            ) : (
            ...
            <div className="flex flex-wrap gap-2">
              {asset.status === 'deployed' && asset.assigned_to_id ? (
              ...
              <Button variant="outline" onClick={() => navigate({ name: 'assets' })}><Edit3 className="w-4 h-4" /> {t('edit')}</Button>
      ...
      >
        <p className="text-sm text-gray-600 mb-3"><span className="font-medium text-gray-900">{asset.name}</span> {t('assignAssetTo')}</p>
